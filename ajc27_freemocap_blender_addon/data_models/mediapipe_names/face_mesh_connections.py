# https://github.com/jonmatthis/mediapipe/blob/212f110c65818db7f2d08c42a9539ecb2b6c0e1d/mediapipe/python/solutions/face_mesh_connections.py
...
def get_mediapipe_face_mesh_connections():
    return deepcopy(_FACEMESH_TESSELATION)

def create_mediapipe_face_mesh_faces(connections: List[Tuple[int, int]] = _FACEMESH_TESSELATION):
    tris = []
    tri = []
    for id in range(len(connections)):
        tri.append(connections[id])
        if len(tri) == 3:
            # TODO - make sure this method correctly makes the normals
            tris.append(tuple([tri[0][0], tri[1][0], tri[2][0]]))
            tri = []
    return tris


if __name__ == "__main__":
    print(create_mediapipe_face_mesh_faces())