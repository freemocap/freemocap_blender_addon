...
def composite_video(
    ...
    for index, camera in enumerate(EXPORT_PROFILES[export_profile]['render_cameras']):
        ...
        video_filepath = str(Path(recording_folder) / video_file_name)

        #  Get a reference to the render_camera object
        ...
        video_node.clip = bpy.data.movieclips.load(video_filepath)
