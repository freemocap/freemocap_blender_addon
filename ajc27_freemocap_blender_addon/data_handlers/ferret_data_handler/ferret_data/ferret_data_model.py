...
from ajc27_freemocap_blender_addon.data_handlers.ferret_data_handler.ferret_data.helpers.ferret_component_data import DataComponent
from ajc27_freemocap_blender_addon.data_handlers.ferret_data_handler.ferret_data.helpers.ferret_data_paths import FerretDataPaths
from ajc27_freemocap_blender_addon.data_handlers.ferret_data_handler.ferret_data.helpers.ferret_data_stats import FerretDataStats
from ajc27_freemocap_blender_addon.skeleton_models.mediapipe_names.mediapipe_trajectory_names import MediapipeTrajectoryNames, \
...
@dataclass
class FerretData:
    body: DataComponent # body marker trajectories, head and body
    toy: DataComponent # toy marker trajectories
    skull: DataComponent # skull marker trajectories and head kinematics (position and orientation) from the head ceres solver
    left_eyeball: DataComponent # left eye-in-head orientation, assumed centered on `left_eye` skull trajectory
    right_eyeball: DataComponent # right eye-in-head orientation, assumed centered on `right_eye` skull trajectory
    left_gaze_vector: DataComponent # left eye gaze vector, computed from left_eyeball and left_eye skull trajectory. Assumed origin at left_eye skull trajectory
    right_gaze_vector: DataComponent # right eye gaze vector, computed from right_eyeball and right_eye skull trajectory. Assumed origin at right_eye skull trajectory
    metadata: dict = field(default_factory=dict) # recording info and whatnot
    ...
    def from_data(cls,
                  ...
                  other: Optional[Dict[str, Union[DataComponent, Dict[str, Any]]]] = None,
                  metadata: Optional[Dict[Any, Any]] = None) -> "FerretData":
    ...
    @classmethod
    def _convert_to_component(cls, other: Dict[str, Union[DataComponent, Dict[str, Any]]]):
        for name, component in other.items():
            if isinstance(component, DataComponent):
                ...
                if not "component_name" in component.keys():
                    ...
                    try:
                        other[name] = DataComponent(**component)
                    except TypeError as e:
                        print(f"Error creating FerretComponentData from dict {component}")
                        raise e
            else:
    ...
    def from_data_paths(cls,
        ...
        return cls.from_data(
            ...
            other={"center_of_mass": DataComponent(name="center_of_mass",
                                                   data=np.load(
    ...
    def from_recording_path(cls,
                            recording_path: str,
                            **kwargs):
        data_paths = FerretDataPaths.from_recording_folder(recording_path)
    ...
    def __str__(self):
        return str(FerretDataStats.from_ferret_data(self))


if __name__ == "__main__":

    recording_path_in = get_path_to_sample_data()
    ferret_data = FerretData.from_recording_path(recording_path=recording_path_in,
                                                       type="original")
    print(str(ferret_data))
