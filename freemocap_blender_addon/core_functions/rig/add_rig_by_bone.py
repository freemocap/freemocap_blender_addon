...
def add_rig_by_bone(
    ...
    while not list(segment_definitions.keys()) <= list(rig.data.edit_bones.keys()):  # check if all bones are in the rig, iterate until all bones are in the rig
        for segment_name, segment in segment_definitions.items():

            if segment_name in rig.data.edit_bones.keys():
                # If the bone already exists, we can skip it
                continue

            if segment.parent not in rig.data.edit_bones.keys() and not pose_definition.value[segment_name].is_root:
            ...
            if pose_definition[segment_name].value.is_root:
                rig_bone.head = mathutils.Vector([0, 0, 0])
            else:
                # Set the bone head position relative to its parent
                rig_bone.head = rig.data.edit_bones[segment.parent].tail
            ...
            rotation_matrix = mathutils.Euler(
                mathutils.Vector(pose_definition[segment_name].value.rotation),
                "XYZ",
            ).to_matrix()
            ...
            rig_bone.parent = rig.data.edit_bones[segment.parent]
            rig_bone.use_connect = pose_definition[segment_name].value.is_connected
