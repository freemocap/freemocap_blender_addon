...
import numpy as np
from freemocap_blender_addon.data_models.freemocap_data.freemocap_data_model import (
...
class FreemocapDataTransformer:
    ...
    def apply_rotations(
        ...
        for rotation_matrix in rotation_matricies:
            self.apply_rotation(
                rotation_matrix=rotation_matrix, component_name=component_name
            )
    ...
    def apply_rotation(
        ...
        if component_name == "other" or component_name is None:
            for name, other_component in self.handler.freemocap_data.other.items():
                other_component.data = self._rotate_component(
                    other_component.data, rotation_matrix
                )
        ...
        if data_frame_name_xyz.shape[-1] == 2:
            ...
            data_frame_name_xyz = np.concatenate(
                [
                    data_frame_name_xyz,
                    np.zeros(
                        (data_frame_name_xyz.shape[0], data_frame_name_xyz.shape[1], 1)
                    ),
                ],
                axis=2,
            )
    ...
    def apply_translations(
        ...
        for frame_number, vector in enumerate(vectors):
            self.apply_translation(
                vector=vector, component_name=component_name, frame_number=frame_number
            )
    ...
    def apply_translation(
        ...
        if component_name == "body" or component_name is None:
            self.handler.body_frame_name_xyz = self._translate_component_data(
                data=self.handler.body_frame_name_xyz,
                translation=vector,
    ...
    def _translate_component_data(
        ...
        if not len(data.shape) in [2, 3]:
            raise ValueError(
                f"Component data must have 2 or 3 dimensions. Got {data.shape[2]} instead."
            )
        if not translation.shape == (3,):
            raise ValueError(
                f"Translation vector must be a 3D vector. Got {translation.shape} instead."
            )
